...
const boundStore = () => cacheStore({ DATASETS, DATASET_JOURNAL });
export {
  boundRestore as restore,
  boundStore as store,
};
...
export function getValue(datasetName: string, key: ?string): ?AnySerializableValue {
...
export function setValue(datasetName: string, key: string, value: ?AnySerializableValue) {
  if (typeof DATASETS[datasetName] === 'undefined') {
  ...
  log.debug('New record', { Key: key, Value: value });
  ...
  log.debug('Writing to journal', updateOp);
...
export async function sync(datasetName: string) {
  const params = {
    DatasetName: datasetName,
    IdentityId: AWS.config.credentials.identityId,
    IdentityPoolId: IDENTITY_POOL_ID,
  ...
  store(DATASETS, DATASET_JOURNAL);
...
export async function init(args : {
  ...
  if (!identityId || !cognitoToken) {
    ...
    try {
      ...
      const getCredentials = promisify(AWS.config.credentials.get);
      ...
      for (const datasetName of datasetsToSync) {
        ...
        log.debug('Resulting records', records);
        DATASETS[datasetName] = records[0];
      }

      log.debug('Completed initialization; resulting datasets =', DATASETS);
    } catch (stsError) {
